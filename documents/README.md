# Documents

Sample document files in various formats.

| File | Format | Description |
|------|--------|-------------|
| sample.pdf | PDF | PDF document with headings, text, and tables |
| sample.docx | DOCX | Microsoft Word document with formatted content |
| sample.pptx | PPTX | PowerPoint presentation with multiple slides |
| sample.odt | ODT | OpenDocument Text (LibreOffice compatible) |
| sample.rtf | RTF | Rich Text Format with basic formatting |
| sample.txt | TXT | Plain text file with sample content |
| sample.log | LOG | Sample application log file with timestamps |
| sample.md | MD | Markdown file with headings, lists, code, and tables |
| sample.rst | RST | reStructuredText document |
| sample.adoc | ADOC | AsciiDoc document |

All documents contain meaningful formatted content including headings, paragraphs, and structured data.
